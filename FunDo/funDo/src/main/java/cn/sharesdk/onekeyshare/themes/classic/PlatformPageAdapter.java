		...
		for (int i = 0; i < logos.length; i++) {
			...
			if (logos[i] == null) {
				...
				if (logos[i] instanceof CustomerLogo) {
					...
					//Log.e("share","name = " + name);
					//resId = ResHelper.getStringRes(tvName.getContext(), name);
					/*if (resId > 0) {
						tvName.setText(name);
					} else {
						tvName.setText("");
					}*/
					if(name.contains("wechatmoments")){
						if(Utils.getLanguage().contains("zh")){
							tvName.setText("微信朋友圈");
						}else {
							tvName.setText("Wechat Moments");
						}
					}else if(name.contains("qzone")){
						if(Utils.getLanguage().contains("zh")) {
							tvName.setText("QQ空间");
						}else{
							tvName.setText("Qzone");
						}
					}else if(name.contains("facebookmessenger")){
						tvName.setText("Facebook Messenger");
					}else if(name.contains("wechat")){
						if(Utils.getLanguage().contains("zh")) {
							tvName.setText("微信");
						}else {
							tvName.setText("Wechat");
						}
					}
				}
			}
