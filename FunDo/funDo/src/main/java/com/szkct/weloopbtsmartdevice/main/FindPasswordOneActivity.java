 ...
 */
public class FindPasswordOneActivity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_findpasswordrone);
                ...
                if (!hasFocus) {
                    ...
                    if (!StringUtils.isEmpty(username)) {
                        if (PublicTools.IsValidMobileNo(username) || StringUtils.isEmail(username)) {
                            ...
                            Toast.makeText(FindPasswordOneActivity.this, "用户名输入内容格式不正确", Toast.LENGTH_SHORT).show();
//                        et_phonenumber.setText("");
                        }
                    } else {
                        Toast.makeText(FindPasswordOneActivity.this, "输入内容不能为空或空格字符", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
...
//        SpeakingFragment.newInstance("").setAudioRecordButton(iv_bottom_talkButton);
    ...
    private void showDialog(final String content) {
            ...
            myDialog = new AlertDialog.Builder(FindPasswordOneActivity.this).create();
            myDialog.show();
            ...
            case R.id.btn_next:
                ...
                if (!StringUtils.isEmpty(username)) {
                    if (PublicTools.IsValidMobileNo(username)) {
                        ...
                        et_phonenumber.setText(username);
                        Intent intent = new Intent(this, EmailVerifiCodeActivity.class);  //当为手机找回密码时，进入FindPasswordTwoActivity，当为邮箱找回密码时，进入邮箱验证码页面
                        intent.putExtra("number",et_phonenumber.getText().toString());
                        startActivity(intent);
                    } else {
                        Toast.makeText(FindPasswordOneActivity.this, "输入内容格式不正确", Toast.LENGTH_SHORT).show();
//                        et_phonenumber.setText("");
                    }
                } else {
                    Toast.makeText(FindPasswordOneActivity.this, "输入内容不能为空或空格字符", Toast.LENGTH_SHORT).show();
