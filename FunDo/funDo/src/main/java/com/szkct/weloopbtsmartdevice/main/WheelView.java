...
public class WheelView extends View {
	private int pading_top_bottom = 50;
	...
	private static final int VALUE_TEXT_COLOR = 0xff010101;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xff474551;

	/** Top and bottom shadows colors */

	private static final int[] SHADOWS_COLORS = new int[] { 0x11c2eb, 0x11c2eb,
		0x11c2eb };

	/** Text size */
	public static int TEXT_SIZE = 45;
	...
	private void initResourcesIfNecessary() {
		...
		if (centerDrawable == null) {
			centerDrawable = new ColorDrawable(0x00000000);
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.EXACTLY) {
			...
			height = getDesiredHeight(itemsLayout) + pading_top_bottom;
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setColor(0xff69eeff);
		Rect rectTop = new Rect();
		rectTop.left = 45;
		rectTop.right = getWidth() - 45;
		rectTop.top = getHeight() / 3;
		rectTop.bottom = getHeight() / 3 + 2;
		canvas.drawRect(rectTop, paint);

		//
		Rect rectBottom = new Rect();
		rectBottom.left = 45;
		rectBottom.right = getWidth() - 45;
		rectBottom.top = getHeight() / 3 * 2;
		rectBottom.bottom = getHeight() / 3 * 2 + 2;
		...
		if (itemsWidth > 0) {
			...
			canvas.translate(PADDING, -ITEM_OFFSET + pading_top_bottom / 2);
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
