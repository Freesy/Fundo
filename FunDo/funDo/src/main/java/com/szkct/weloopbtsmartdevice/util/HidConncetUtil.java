...
//	private BluetoothDevice device;
	...
	private BluetoothProfile mBluetoothProfile; // todo  add lx
	private final int INPUT_DEVICE = 4;   // todo  add lx
	...
	private BluetoothProfile.ServiceListener connect = new BluetoothProfile.ServiceListener() {
		...
		public void onServiceConnected(int profile, BluetoothProfile proxy) {
			...
			try {
//				if (profile == INPUT_DEVICE) {    // todo  add lx
...
//				}
		...
		public void onServiceConnected(int profile, BluetoothProfile proxy) {
			try {
				if (profile == getInputDeviceHiddenConstant()) {
					...
					boolean isDisConnect = (boolean) SharedPreUtil.getParam(BTNotificationApplication.getInstance(), SharedPreUtil.USER, SharedPreUtil.BLE_CLICK_STOP, false);

					if (device != null && isDisConnect) {
	...
	public void connect(final BluetoothDevice bluetoothDevice) {
		...
		try {
			BluetoothAdapter.getDefaultAdapter().getProfileProxy(context, connect, getInputDeviceHiddenConstant());
		} catch (Exception e) {
	...
	public Boolean rename(BluetoothDevice bluetoothDevice, String name) {
		...
		Method createBondMethod;
		try {
			createBondMethod = BluetoothDevice.class.getMethod("setAlias", String.class);
	...
	public void getHidConncetList(
		...
		try {
			BluetoothAdapter.getDefaultAdapter().getProfileProxy(context, getList, getInputDeviceHiddenConstant());
		} catch (Exception e) {
