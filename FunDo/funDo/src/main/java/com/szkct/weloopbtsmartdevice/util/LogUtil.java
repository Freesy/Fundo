...
public class LogUtil {
	...
	void init(Context context) {
		...
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			...
			mLogPath = context.getFilesDir().getAbsolutePath();   // /storage/emulated/0/com.kct.fundo.btnotification
		}
		...
		/*
		 * // Only save MAX_LOG_COUNT logs File[] allLogs = file.listFiles(); if
		 * (allLogs.length > MAX_LOG_COUNT) { Arrays.sort(allLogs, new
		 * FileComparator()); int deleteLogCout = allLogs.length - MAX_LOG_COUNT
		 * + 1; for (int index = 0; index < deleteLogCout; index++) {
		 * allLogs[index].delete(); } }
		 */
	 ...
	 * 
	 * @Override public int compare(File leftFile, File rightFile) { return
	 * leftFile.getName().compareTo(rightFile.getName()); }
	 * 
	 * }
	 */
	public void start() {
	...
	private class LogDumper extends Thread {
		...
		final String filename = "FunDo.txt";  // Method threw 'java.lang.NullPointerException' exception. Cannot evaluate com.szkct.weloopbtsmartdevice.util.LogUtil$LogDumper.toString()

		public LogDumper(String pid, String dir) {
			mPID = pid;
			File logFile = new File(dir, filename);   // /storage/emulated/0/com.kct.fundo.btnotification/FunDo.txt
			if (!logFile.exists()) {
				try {
			...
			// cmds = "logcat -s way";
			mCmds = "logcat *:e *:w *:i *:d *:v | grep \"(" + mPID + ")\"";
			...
			try {
				...
				while (mRunning) {
					...
					if (mLogWriter != null && line.contains(mPID)) {
						mLogWriter.write((StringUtils.getStrDate() + "  " + line + "\n"));
					}
				}
