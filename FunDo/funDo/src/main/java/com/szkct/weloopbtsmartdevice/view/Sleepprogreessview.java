...
public class Sleepprogreessview extends View {
  ...
  /*  private static final int[] SECTION_COLORS = {Color.GREEN, Color.YELLOW,
   ...
   // final int mBarInterval = 7;//TODO 每段间隔
    ...
    final float nPaintWidth = (float) 6;//TODO 
    private float maxCount = 60;
    private float currentCount = 60;
    ...
    public Sleepprogreessview(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.mContext=context;
        init(context);
    }
    
    public Sleepprogreessview(Context context, AttributeSet attrs) {
    ...
    private void init(Context context) {
        ...
        mPaint.setColor( a.getColor(0,Color.parseColor("#37EEEA")));
     ...
     //   BlurMaskFilter n2m=new BlurMaskFilter(10, BlurMaskFilter.Blur.SOLID);
      //  n2Paint.setMaskFilter(n2m);
        ...
        BlurMaskFilter am=new BlurMaskFilter(2, BlurMaskFilter.Blur.SOLID);
    ...
    //    LinearGradient shader = new LinearGradient(0, mHeight, 0, 0 , SECTION_COLORS, null,Shader.TileMode.MIRROR);
    //    mPaint.setShader(shader);

        for (int i = 0; i < 60; ++i) {
        	if(i<currentCount){
        		canvas.drawArc(rectBlackBg, (float) (i * 6-90), mBarWidth, false, m2Paint);
        	}else{
        		canvas.drawArc(rectBlackBg, (float) (i * 6-90), mBarWidth, false, mPaint);
        	}
      ...
      //  mPaint.reset();
       // Log.e("currentCount", currentCount+"du");
        canvas.drawArc(nrectBlackBg, -90,(float) (currentCount*6), false, n2Paint);
        canvas.drawArc(nrectBlackBg, (float) (currentCount*6-90),(float) ((60-currentCount)*6), false, nPaint);
        canvas.drawArc(nrectBlackBg, (float) (currentCount*6-90),(float) 0.1, false, aPaint);
      //  nPaint.reset();
    }
